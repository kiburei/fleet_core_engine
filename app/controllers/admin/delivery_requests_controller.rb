class Admin::DeliveryRequestsController < AdminController
  ...
  def destroy
    ...
    redirect_to admin_delivery_requests_path
  end
  ...
  def bulk_actions
    ...
    when 'cancel'
      reason = params[:cancellation_reason] || 'Bulk cancelled by admin'
      cancelled_count = 0
      delivery_ids.each do |id|
        delivery = DeliveryRequest.find(id)
  ...
  def create_marketplace_order
    Marketplace::Order.create!(
      ...
      order_number: "ADM-#{Time.current.strftime('%Y%m%d')}-#{SecureRandom.hex(3).upcase}",
